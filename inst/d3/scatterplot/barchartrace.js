...
const longestLabel =
    data.map(d => d[options.y]).sort((a, b) => b.length - a.length)
...
function update(frame, init = false) {
  ...
  frameData.sort(function(
      a, b) { return d3[options.sort](a[options.x], b[options.x]); })
  ...
  yScale.domain(d3.reverse(yLabels))
      .paddingInner(options.paddingWidth)
      .paddingOuter(options.paddingWidth)
  colorScale.domain(yLabels).range(
      d3["scheme" + options.colorCategory].slice(0, yLabels.length))

  // draw bars

  layerBar.selectAll(".bar")
    ...
    .call(
      g => {
        ...
        if (options.timeLabel) {
          ...
          timeLabelText.text(options.timeLabelOpts.prefix + frame +
                             options.timeLabelOpts.suffix)
        ...
        ${
                  dims.panel.width -
                  (timeLabelDims.width * options.timeLabelOpts.xOffset)},
        ${
                  dims.panel.height -
