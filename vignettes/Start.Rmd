...
This function allows you to create an animated bar chart race. It's similar to `barChart` but takes an additional variable mapped to the `time` dimension, with options for styling transitions.

Let's make a bar chart race of population growth among various countries with a subset of the `gapminder` dataset from the [`{gapminder}` package](https://github.com/jennybc/gapminder):
...
In this simplest example, we simply pass call `barChartRace()` like `barChart()` but with an additional variable mapped to the time dimension specified with `time = year`:
